# version：启用QTHREAD，并且使用信号槽来进行两个控件之间的信号传输，这样可以避免窗口的异常退出
import ctypes
import threading
import numpy as np
import cv2
...
so_file_path = os.path.join(base_dir, 'libudp_dll.so')
dll = ctypes.CDLL(so_file_path)
...
class MainWindow(QMainWindow,Ui_LWIR):
    ...
    def stream_and_update(self):
        ...
        #print(self.frame_base.min(),self.frame_base.max())
